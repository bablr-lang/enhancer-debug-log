...
import * as matchableLang from '@bablr/miniparser-language-cstml-matchable';
...
const formatMatch = (instruction) => {
  ...
  return `${formattedEffects} ${print(matchableLang, matchable)}`;
};

const formatDisambiguate = (cases) => {
  return cases.map(([matchable]) => print(matchableLang, matchable)).join(' ');
};
