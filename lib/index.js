import { print } from '@bablr/miniparser';
import { memoize, mapProductions, mapGrammars } from '@bablr/helpers/enhancers';
import { miniprinterGrammar as matchableGrammar } from '@bablr/miniparser-language-cstml-matchable';
...
const formatMatch = (instruction) => {
  ...
  return `${formattedEffects} ${print(matchableGrammar, matchable)}`;
};

const formatDisambiguate = (cases) => {
  return cases.map(([matchable]) => print(matchableGrammar, matchable)).join(' ');
};
