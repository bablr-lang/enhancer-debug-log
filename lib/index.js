...
import { dependencies } from '@bablr/language-en-cstml-json';
...
const writeError = (text) => {
  return buildCall(
    buildIdentifier('write'),
    buildTuple(
      buildTupleValues([
        buildString(text),
        buildObject(
          buildObjectProperties([buildProperty(buildIdentifier('stream'), buildNumber(2))]),
        ),
      ]),
    ),
  );
    ...
    *[type](props) {
      ...
      try {
        ...
        while (!current.done) {
          ...
          yield writeError(`>>> ${printSource(instr)}`);

          const eats =
            printType(getRoot(instr).type) === 'Call' &&
            ['eat', 'eatMatch'].includes(getCooked(getRoot(instr).properties.verb.node));
