...
import { spamexLanguage } from '@bablr/helpers/languages';
...
const formatMatch = (instruction) => {
  ...
  return `${formattedEffects} ${print(spamexLanguage, matchable)}`;
};

const formatDisambiguate = (cases) => {
  return cases.map(([matchable]) => print(spamexLanguage, matchable)).join(' ');
};
